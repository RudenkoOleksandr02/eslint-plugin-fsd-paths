...
const { isPathRelative, stripAlias, getCurrentFileLayer, getImportLayer, layers } = require("../helpers");
    ...
    return {
      ImportDeclaration(node) {
        ...
        const isIgnored = ignoreImportPatterns.some(pattern => (
            ...
            micromatch.isMatch(withoutAlias, pattern)
        ));
        if (isIgnored) return;
        ...
        if (!layers[importLayer] || !layers[currentFileLayer]) return;
